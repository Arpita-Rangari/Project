...
public class ShoopingCartService implements IShoppingcartService {
	...
	{
		List<Cart> cartList = cartrepo.findByStudent(user);
		...
		for (Cart cart: cartList) {
	            ...
	            totalcost  += 1 * cart.getProduct().getPrice();
	...
	public void addProduct(AddToCartDto cartdto, User user )
	{
		...
		//cart.setQuantity(cartdto.getQuantity());  removed
...
System.out.println(cartdetails.getStudent().getId());
System.out.println(user.getId());

	        if (cartdetails.getStudent().getId()!= user.getId()) {
	...
	@Override
	public String updatecart(int id, int pid, User user) {
		//get the product
