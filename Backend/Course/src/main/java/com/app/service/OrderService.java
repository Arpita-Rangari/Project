...
public class OrderService  implements IOrderService{
	...
	public OrderResponseDto  placeOrderDetails(OrderDto orderdto ,User user) {
		List<Cart> list = cartrepo.findByStudent(user); //get the user ----->cart items
...
//		service.updateproductQuantity(pid, userq);
	...
	@Override
	public List<OrderResponseDto> getStudentDetails(long id) {
	...
	for(Order order :list)
	{
		...
		int diff =Period.between(orderdate, deliverydate).getDays();
		System.out.println(diff);
		if(diff==7)
		{
			response.setMsg("Your order will be delivered on " + deliverydate);
		}
		if(diff==6)
		{
			response.setMsg("Your order will be delivered on " + deliverydate);
