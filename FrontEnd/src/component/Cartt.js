...
const Cartt = () => {
  ...
  return (
    <div>
      ...
      <h5>
        In Your Cart : <b>{cartArr.length}</b>
      </h5>
        ...
        <tbody>
          {cartArr.map((cart) => (
            <tr>
              ...
              {/* <td>{cart.product.quantity * cart.product.price} </td>removed */}
              <td>{1 * cart.product.price} </td>
            </tr>
          ))}
 ...
 <hr/> */}
      <center>
        <Link className="btn btn-info" style={{ width: "200px" }} to={`/order`}>
          Place Order
        </Link>
      </center>
