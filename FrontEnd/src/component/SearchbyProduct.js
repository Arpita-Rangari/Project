...
const SearchbyProduct = () => {
    ...
    productService
      ...
      .then(response => {
        ...
        let id = (response.data).map(function (p) {
          if (p.productName == key) return p.id;
          else return 0;
        })
        function checkcourseid(id) {
          return id > 0;
        }
        const result = id.filter(checkcourseid);
        //alert("result" + result);
        //console.log(id);
        ...
        productService
          ...
          .then((response) => {
            ...
            // setQuantity(product.quantity); removed
  ...
  const routeCart = (id) => {
    ...
    if (1 == 2) {
        ...
        .then(response => {
          ...
          console.log(
            "dataaaaaaaaaaaaaa--------->" +
              ...
              data.courseId +
              "-------" +
              arr.includes(data.courseId)
          )
          console.log("Printing cost data", response.data);

          if (arr.includes(data.courseId)) {
      ...
      console.log("dataaaaaaaaaaaaaa--------->" +arr[0] +arr[1] +"----" +data.courseId +"-------" +arr.includes(data.courseId))
    }
  }
  useEffect(() => {
    init();
        ...
        <Row>
          <Col xs={12} md={4} lg={3}>
            <Card>
              ...
              <Card.Body>
                <Card.Title>Course Title : {product.productName}</Card.Title>
